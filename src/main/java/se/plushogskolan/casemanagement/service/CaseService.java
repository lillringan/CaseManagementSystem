...
public class CaseService {
	...
	public Slice<User> getAllUsers(Pageable page) {
		try {
	...
	public User addUserToTeam(Long userId, Long teamId) {
		try {
			...
			throw new ServiceException("User could not be added to Team", e);
		}
	}
	...
	public Team save(Team team) {
		if (!isPersistedObject(team)) {
			try {
				...
				throw new ServiceException("Team could not be saved", e);
			}
		} else {
	...
	public Team inactivateTeam(Long teamId) {
		...
		if (team.isActive() == true) {
			try {
				...
				throw new ServiceException("Team could not be inactivated", e);
			}
		} else {
	...
	public Team activateTeam(Long teamId) {
		try {
			...
			throw new ServiceException("Team could not be activated", e);
		}
	}
	...
	public Team searchTeamByName(String name) {
		try {
			...
			throw new ServiceException("Could not get team with name: " + name, e);
		}
	}
	...
	public Slice<Team> getAllTeams(Pageable pageable) {
		try {
			...
			throw new ServiceException("Could not get teams", e);
		}
	}
	...
	public WorkItem getWorkItem(Long id) {
		try {
			...
			throw new ServiceException("Could not get Workitem with id: " + id, e);
		}
	}
	...
	public Page<WorkItem> getWorkItemsByPeriodAndStatus(WorkItem.Status status, LocalDate start, LocalDate end,
			Pageable pageable) {
		try {
			return workItemRepository.getWorkItemsByStatusAndPeriod(status, start, end, pageable);
		} catch (DataAccessException e) {
			throw new ServiceException("Could not get workitems between: " + start + " and " + end, e);
		}
	}

	public Slice<WorkItem> getAllWorkItems(Pageable page) {
		try {
	...
	private boolean userHasSpaceForAdditionalWorkItem(Long workItemId, Long userId, Pageable pageable) {
		...
		if (workItems.getNumberOfElements() < 5) {
			...
			System.out.println(workItems.getSize());
			return false;
		}
	}
